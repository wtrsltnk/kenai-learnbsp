...
studiohdr_t *StudioModel::LoadModel( Data& file, fs::FileSystem& filesystem )
{
	byte* buffer = new byte[file.dataSize];
	memcpy(buffer, file.data, file.dataSize);
...
studioseqhdr_t *StudioModel::LoadDemandSequences( Data& file, fs::FileSystem& filesystem )
{
	byte* buffer = new byte[file.dataSize];
	memcpy(buffer, file.data, file.dataSize);
...
void StudioModel::Init( Data& file, fs::FileSystem& filesystem )
{
	m_pstudiohdr = LoadModel( file, filesystem );
	...
	if (m_pstudiohdr->numtextures == 0)
	{
		...
		strcpy( texturename, file.name );
		strcpy( &texturename[strlen(texturename) - 4], "T.mdl" );

		Data tex;
		if (filesystem.openFile(tex, texturename))
		{
			m_ptexturehdr = LoadModel( tex, filesystem );
		}
	}
	...
	if (m_pstudiohdr->numseqgroups > 1)
	{
		for (int i = 1; i < m_pstudiohdr->numseqgroups; i++)
		{
			...
			strcpy( seqgroupname, file.name );
			sprintf( &seqgroupname[strlen(seqgroupname) - 4], "%02d.mdl", i );

			Data seq;
			if (filesystem.openFile(seq, seqgroupname))
			{
				m_panimhdr[i] = LoadDemandSequences( seq, filesystem );
			}
		}
	}
}
